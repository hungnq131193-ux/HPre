...
| Deterministic build gate | N/A | N/A | Local Windows build | Debug APK | Pass | 546 unit tests passed with 0 failures, lint passed with 0 errors, AndroidTest sources compiled, and debug APK assembled on 2026-08-27. |
...
# Video experience optimization verification (2026-08-27)

Automated verification is pending a local JDK 17 runtime. Do not mark the device-only rows PASS from unit tests.

| Check | Environment / procedure | Result |
|---|---|---|
| Time to first rendered frame | Same device/network, record Watch open → first rendered frame before/after | PENDING DEVICE |
| Rebuffer count and duration | Play the same 10-minute video under a controlled bandwidth profile | PENDING DEVICE |
| Adaptive down/up switch | Multi-variant HLS/DASH fixture; throttle then restore bandwidth | PENDING DEVICE |
| Quality cap | Set max height and confirm effective Media3 track never exceeds it | PENDING DEVICE |
| Watch → mini surface handoff | Perform three consecutive portrait swipe-down handoffs; record visible black-frame gaps | PENDING DEVICE |
| 100-row Home/Watch scroll | Record frame timing and heap while scrolling complete feeds | PENDING DEVICE |
| Activity recreation | Rotate/recreate during Watch and mini-player playback | PENDING DEVICE |
| System PiP enter/exit | Leave app while eligible, return, confirm position/play state/surface | PENDING DEVICE |
