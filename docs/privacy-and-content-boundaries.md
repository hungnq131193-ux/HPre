# FlowTube Privacy & Content Boundaries

## 1. Non-Official Extractor & Technical Scope
- FlowTube V1 targets **NewPipeExtractor** (`com.github.TeamNewPipe:NewPipeExtractor:v0.26.5`) as a non-official client extractor library behind an isolated provider-neutral adapter.
- FlowTube is an independent client and is **not** endorsed by, affiliated with, or an official product of YouTube, Google LLC, or NewPipe.
...
- **No Access Control Bypass:** FlowTube **never** bypasses, defeats, or evades login barriers, paywalls, DRM (Digital Rights Management), CAPTCHA challenges, age verifications, or geographic geo-fencing.
...
- **No Tracking or Analytics:** FlowTube contains **no** telemetry, ads, tracking pixels, crash reporters (e.g. Firebase Analytics, Crashlytics), or remote user identifier beacons.
- **No User Account / Source Mutation:** FlowTube does not offer source-platform login and does not mutate remote source-platform account states (likes, subscriptions, comments).
...
- FlowTube presents an original Jetpack Compose Material 3 interface.
