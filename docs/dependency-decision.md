# HPre Dependency Decisions & Provenance

This document records the exact coordinates, repositories, licenses, release dates, provenance sources, and compatibility rationale for all build toolchain components and runtime dependencies locked for HPre V1 (Task 1 Build Gate).

> **Verification Status (Updated 2026-08-26):** Local dependency resolution, toolchain compatibility, 435 debug unit tests, lint, AndroidTest source compilation, and debug APK assembly were verified using session-scoped Eclipse Temurin `17.0.14+7` and Android SDK API 35. `gradlew.bat clean testDebugUnitTest lintDebug assembleDebug compileDebugAndroidTestKotlin --no-daemon` completed successfully. The generated debug APK is 24,890,436 bytes with SHA-256 `F0A83AA126919996AAB5E93372D46E6C6000186C26253DC84994DB4C78902BFE`.
>
