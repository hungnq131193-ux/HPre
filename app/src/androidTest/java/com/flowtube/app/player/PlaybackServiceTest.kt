...
class PlaybackServiceTest {
    ...
    fun controller_reconnect_keeps_one_session_media_item_and_position() = runBlocking {
        ...
        val streamInfo = StreamInfo(
            ...
            videoStreams = listOf(
                VideoStream(
                    url = "https://flowtube.test/prog.mp4",
                    format = "mp4",
                    resolution = "720p",
                    width = 1280,
                    height = 720,
    ...
    fun notification_channel_is_registered() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        FlowTubeMediaNotification.ensureNotificationChannel(context)

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val nm = context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
            val channel = nm.getNotificationChannel(FlowTubeMediaNotification.CHANNEL_ID)
            assertNotNull(channel)
            assertEquals(FlowTubeMediaNotification.CHANNEL_NAME, channel.name)
    ...
    fun quality_selection_success_and_invalid_rejected_protocol() = runBlocking {
        ...
        val streamInfo = StreamInfo(
            ...
            videoStreams = listOf(
                VideoStream(
                    url = "https://flowtube.test/720.mp4",
                    format = "mp4",
                    resolution = "720p",
                    width = 1280,
                    height = 720,
                ...
                VideoStream(
                    url = "https://flowtube.test/1080.mp4",
                    format = "mp4",
                    resolution = "1080p",
                    width = 1920,
                    height = 1080,
        ...
        val fakeFuture = withContext(Dispatchers.Main) {
            androidx.media3.session.MediaController.Builder(
                ...
                androidx.media3.session.SessionToken(
                    context,
                    android.content.ComponentName(context, FlowTubePlaybackService::class.java)
                )
            ).buildAsync()
        }
