...
class PlayerPackageBoundaryTest {
    ...
    fun player_package_main_and_test_sources_do_not_import_extractor_or_org_schabi() {
        ...
        val playerMainDir = File(mainSourceRoot, "com/flowtube/app/player")
        val playerTestDir = File(testSourceRoot, "com/flowtube/app/player")
        ...
        val forbiddenPattern = Regex("""\b(com\.flowtube\.app\.extractor|org\.schabi)\b""")
    ...
    fun ui_watch_package_does_not_directly_import_or_invoke_raw_exoplayer() {
        val mainSourceRoot = findMainSourceRoot()
        val watchUiDir = File(mainSourceRoot, "com/flowtube/app/ui/watch")
    ...
    @Test
    fun flowtube_playback_service_is_sole_exoplayer_owner_and_releaser_in_main_sources() {
        val mainSourceRoot = findMainSourceRoot()
        ...
        for (file in allMainFiles) {
            if (file.name == "FlowTubePlaybackService.kt") {
                continue
            }
            file.readLines().forEachIndexed { index, line ->
                val trimmed = line.trim()
        ...
        if (violations.isNotEmpty()) {
            fail("Only FlowTubePlaybackService is allowed to construct ExoPlayer or release player in main source:\n" + violations.joinToString("\n"))
        }
    }
}
